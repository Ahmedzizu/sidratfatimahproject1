      ...
      return (
        <>
          ...
          <Button
            ...
            style={{ fontFamily: 'Cairo', margin: '0 5px', backgroundColor: '#808080', color: '#FFFFFF' }}
          >
...
if (entityFilter !== 'all') {
  ...
  const counts = {
    all: data.filter(row => row.type === 'chalet' && row.status !== 'unConfirmed').length,
    confirmed: data.filter(row => row.type === 'chalet' && row.status === 'confirmed' && !row.deferred).length,
    deferred: data.filter(row => row.type === 'chalet' && row.status === 'confirmed' && row.deferred).length,
    canceled: data.filter(row => row.type === 'chalet' && row.status === 'canceled').length,
    completed: data.filter(row => row.type === 'chalet' && row.completed === true).length,
  };
                    ...
                    <span style={{ color: '#1976d2', fontSize: '18px', fontWeight: 'bold' }}>
                      {row.cost.toLocaleString()}
                             ...
                             backgroundColor:
                                 ...
                                 : row.status === "confirmed" && row.isModified ? "#b6bdb9ff"
